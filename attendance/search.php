<?php
session_start();
if(!isset($_SESSION['status'])) $_SESSION['where'] = " WHERE salary_type=1 AND register=1";
if(!isset($_SESSION['order'])) $_SESSION['order'] = " ORDER BY staff_id DESC";
if(isset($_POST['search'])) {
	$_SESSION['status'] = $_POST['status'];
	if($_SESSION['status'] == "enroll") {
		$where = " WHERE salary_type=1 AND register=1";
	} elseif($_SESSION['status'] == "retire") {
		$where = " WHERE salary_type=1 AND register=0";
	} elseif($_SESSION['status'] == "all") {
		$where = " WHERE salary_type=1 AND register IS NOT NULL";
	}
